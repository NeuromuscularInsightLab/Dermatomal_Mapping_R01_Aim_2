...
for run in ${runs[@]}; do
    for subject in ${subjects[@]}; do
        ...
        # zscore_sc=`fslstats ${data_path}/preprocessing_ALL/${subject}/ses-${session}spinalcord/func/run-${run}/${subject}_ses-${session}spinalcord_task-${task}_run-${run}_bold_mc2_pnm_stc2template_smooth225_trialwise_second_level.gfeat/cope1.feat/stats/cope1.nii.gz -k ${data_path}/masks/mask_n25.nii.gz -M`
        # voxels_sc=`fslstats ${data_path}/preprocessing_ALL/${subject}/ses-${session}spinalcord/func/run-${run}/${subject}_ses-${session}spinalcord_task-${task}_run-${run}_bold_mc2_pnm_stc2template_smooth225_trialwise_second_level.gfeat/cope1.feat/stats/cope1.nii.gz -k ${data_path}/masks/mask_n25.nii.gz -V | cut -d " " -f1`
        # left_voxels_sc=`fslstats ${data_path}/preprocessing_ALL/${subject}/ses-${session}spinalcord/func/run-${run}/${subject}_ses-${session}spinalcord_task-${task}_run-${run}_bold_mc2_pnm_stc2template_smooth225_trialwise_second_level.gfeat/cope1.feat/stats/cope1.nii.gz -k ${data_path}/masks/rois_n25/left_sc_mask.nii.gz -V | cut -d " " -f1`
        # right_voxels_sc=`fslstats ${data_path}/preprocessing_ALL/${subject}/ses-${session}spinalcord/func/run-${run}/${subject}_ses-${session}spinalcord_task-${task}_run-${run}_bold_mc2_pnm_stc2template_smooth225_trialwise_second_level.gfeat/cope1.feat/stats/cope1.nii.gz -k ${data_path}/masks/rois_n25/right_sc_mask.nii.gz -V | cut -d " " -f1`
...
mv subject_metrics_cope.txt ${data_out}/subject_metrics.txt